// Auto-generated by buffela-kotlin, DO NOT MODIFY
@file:Suppress(
    "USELESS_CAST", 
    "SimpleRedundantLet", 
    "RemoveEmptySecondaryConstructorBody", 
    "PropertyName",
    "RemoveRedundantQualifierName",
    "RedundantVisibilityModifier"
)

package gr.elaevents.buffela.examples.authToken

import gr.elaevents.buffela.serialization.utils.assertLength
import gr.elaevents.buffela.serialization.utils.assertSize
import gr.elaevents.buffela.serialization.Serializable as _Serializable
import gr.elaevents.buffela.serialization.SerializerBuffer as _SerializerBuffer
import gr.elaevents.buffela.deserialization.utils.invalidSubtype
import gr.elaevents.buffela.deserialization.Deserializer as _Deserializer
import gr.elaevents.buffela.deserialization.DeserializerBuffer as _DeserializerBuffer

enum class Gender: _Serializable {
    FEMALE,
    MALE,
    ;
    
    override fun serialize(buffer: _SerializerBuffer) {
        buffer.writeUInt(this.ordinal.toUInt(), 1)
    }
    
    companion object Deserializer: _Deserializer<Gender> {
        override fun deserialize(buffer: _DeserializerBuffer): Gender {
            return Gender.entries[buffer.readUInt(1).toInt()]
        }
    }
}

class Phone: _Serializable {
    val countryCode: kotlin.UInt
    val number: kotlin.String
    
    constructor(
        countryCode: kotlin.UInt,
        number: kotlin.String,
    ): super() {
        this.countryCode = countryCode
        this.number = number
    }
    
    override fun serialize(buffer: _SerializerBuffer) {
        buffer.writeUInt(this.countryCode, 10)
        buffer.writeString(this.number)
        buffer.writeByte(0)
    }
    
    internal constructor(buffer: _DeserializerBuffer) {
        this.countryCode = buffer.readUInt(10)
        this.number = buffer.readString()
    }
    
    companion object Deserializer: _Deserializer<Phone> {
        override fun deserialize(buffer: _DeserializerBuffer): Phone {
            return Phone(buffer)
        }
    }
}

sealed class User: _Serializable {
    protected val _userId: kotlin.String
    open val userId get() = this._userId
    
    constructor(
        userId: kotlin.String,
    ): super() {
        this._userId = userId
    }
    
    protected fun serializeLeafIndex(buffer: _SerializerBuffer, index: Int) {
        buffer.writeUInt(index.toUInt(), 2)
    }
    
    protected open fun userId(buffer: _SerializerBuffer) {
        assertLength(36, this.userId.length)
        buffer.writeString(this.userId)
    }
    
    override fun serialize(buffer: _SerializerBuffer) {
        this.userId(buffer)
    }
    
    protected open fun userId(buffer: _DeserializerBuffer) = buffer.readString(36)
    
    protected constructor(buffer: _DeserializerBuffer) {
        this._userId = this.userId(buffer)
    }
    
    companion object Deserializer: _Deserializer<User> {
        override fun deserialize(buffer: _DeserializerBuffer): User {
            return when(val index = buffer.readUInt(2).toInt()) {
                0 -> Anonymous(buffer)
                1 -> Registered.Viewer(buffer)
                2 -> Registered.Organizer(buffer)
                else -> invalidSubtype(index)
            }
        }
    }
    
    class Anonymous: User {
        constructor(
            userId: kotlin.String,
        ): super(
            userId,
        ) {}
        
        override fun serialize(buffer: _SerializerBuffer) {
            this.serializeLeafIndex(buffer, 0)
            super.serialize(buffer)
        }
        
        internal constructor(buffer: _DeserializerBuffer): super(buffer) {}
    }
    
    sealed class Registered: User {
        val verified: kotlin.Boolean
        
        constructor(
            userId: kotlin.String,
            verified: kotlin.Boolean,
        ): super(
            userId,
        ) {
            this.verified = verified
        }
        
        override fun serialize(buffer: _SerializerBuffer) {
            super.serialize(buffer)
            buffer.writeBoolean(this.verified)
        }
        
        protected constructor(buffer: _DeserializerBuffer): super(buffer) {
            this.verified = buffer.readBoolean()
        }
        
        class Viewer: Registered {
            val birthDate: gr.elaevents.buffela.examples.authToken.Date
            val gender: gr.elaevents.buffela.examples.authToken.Gender
            val phone: gr.elaevents.buffela.examples.authToken.Phone?
            
            constructor(
                userId: kotlin.String,
                verified: kotlin.Boolean,
                birthDate: gr.elaevents.buffela.examples.authToken.Date,
                gender: gr.elaevents.buffela.examples.authToken.Gender,
                phone: gr.elaevents.buffela.examples.authToken.Phone?,
            ): super(
                userId,
                verified,
            ) {
                this.birthDate = birthDate
                this.gender = gender
                this.phone = phone
            }
            
            override fun serialize(buffer: _SerializerBuffer) {
                this.serializeLeafIndex(buffer, 1)
                super.serialize(buffer)
                buffer.writeDate(this.birthDate)
                this.gender.serialize(buffer)
                buffer.writeBoolean(this.phone != null)
                this.phone?.let {
                    it.serialize(buffer)
                }
            }
            
            internal constructor(buffer: _DeserializerBuffer): super(buffer) {
                this.birthDate = buffer.readDate()
                this.gender = Gender.deserialize(buffer)
                this.phone = if (buffer.readBoolean()) Phone.deserialize(buffer) else null
            }
        }
        
        class Organizer: Registered {
            val roles: Array<kotlin.String>
            override val userId get() = _userId as kotlin.String
            val email: kotlin.String
            
            constructor(
                userId: kotlin.String,
                verified: kotlin.Boolean,
                roles: Array<kotlin.String>,
                email: kotlin.String,
            ): super(
                userId,
                verified,
            ) {
                this.roles = roles
                this.email = email
            }
            
            override fun userId(buffer: _SerializerBuffer) {
                buffer.writeString(this.userId)
                buffer.writeByte(0)
            }
            
            override fun serialize(buffer: _SerializerBuffer) {
                this.serializeLeafIndex(buffer, 2)
                super.serialize(buffer)
                buffer.writeUByte(this.roles.size.toUByte())
                for (item1 in this.roles) {
                    buffer.writeString(item1)
                    buffer.writeByte(0)
                }
                buffer.writeString(this.email)
                buffer.writeByte(0)
            }
            
            override fun userId(buffer: _DeserializerBuffer) = buffer.readString()
            
            internal constructor(buffer: _DeserializerBuffer): super(buffer) {
                this.roles = Array(buffer.readUByte().toInt()) { _ -> buffer.readString() }
                this.email = buffer.readString()
            }
        }
    }
}

class AuthTokenPayload: _Serializable {
    val issuedAt: kotlin.Double
    val user: gr.elaevents.buffela.examples.authToken.User
    
    constructor(
        issuedAt: kotlin.Double,
        user: gr.elaevents.buffela.examples.authToken.User,
    ): super() {
        this.issuedAt = issuedAt
        this.user = user
    }
    
    override fun serialize(buffer: _SerializerBuffer) {
        buffer.writeDouble(this.issuedAt)
        this.user.serialize(buffer)
    }
    
    internal constructor(buffer: _DeserializerBuffer) {
        this.issuedAt = buffer.readDouble()
        this.user = User.deserialize(buffer)
    }
    
    companion object Deserializer: _Deserializer<AuthTokenPayload> {
        override fun deserialize(buffer: _DeserializerBuffer): AuthTokenPayload {
            return AuthTokenPayload(buffer)
        }
    }
}

class AuthTokenSignature: _Serializable {
    val hmac256: kotlin.ByteArray
    
    constructor(
        hmac256: kotlin.ByteArray,
    ): super() {
        this.hmac256 = hmac256
    }
    
    override fun serialize(buffer: _SerializerBuffer) {
        assertSize(32, this.hmac256.size)
        buffer.writeBytes(this.hmac256)
    }
    
    internal constructor(buffer: _DeserializerBuffer) {
        this.hmac256 = buffer.readBytes(32)
    }
    
    companion object Deserializer: _Deserializer<AuthTokenSignature> {
        override fun deserialize(buffer: _DeserializerBuffer): AuthTokenSignature {
            return AuthTokenSignature(buffer)
        }
    }
}
